...
class EncodingTest extends FunSpec with Matchers with OptionValues {
  ...
  describe("amino header, length prefixed") {
    val aminoHeader = ProtobufConverter.toAmino(block.header)
  ...
  describe("amino block") {
    it("encode data") {
      val simpleBytes = Protobuf.encode(ProtobufConverter.toAmino(block.data))
      val goSize = 258
      goSize shouldBe simpleBytes.length
    ...
    it("encode commit") {
      val simpleBytes = Protobuf.encode(ProtobufConverter.toCommit(block.last_commit))
      val goSize = 629
    ...
    it("encode block") {
      val scalaHex = toHex(Protobuf.encodeLengthPrefixed(ProtobufConverter.toAmino(block)))
      val goHex =
  ...
  describe("canonical") {
    ...
    it("encode canonical vote") {
      ...
      val bytes = Protobuf.encodeLengthPrefixed(Signature.canonicalize(vote, chainID))
