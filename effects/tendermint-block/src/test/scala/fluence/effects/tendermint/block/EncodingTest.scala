...
class EncodingTest extends FunSpec with Matchers with OptionValues {
  ...
  describe("amino header, length prefixed") {
    val aminoHeader = AminoConverter.toAmino(block.header)
    ...
    it("encode header") {
      val simpleBytes = Amino.encode(aminoHeader)
      val goSize = 363
      goSize shouldBe simpleBytes.length

      val scalaHex = toHex(Amino.encodeLengthPrefixed(aminoHeader))
      val goHex =
  ...
  describe("amino block") {
    it("encode data") {
      val simpleBytes = Amino.encode(AminoConverter.toAmino(block.data))
      val goSize = 258
      goSize shouldBe simpleBytes.length
    ...
    it("encode commit") {
      val simpleBytes = Amino.encode(AminoConverter.toCommit(block.last_commit))
      val goSize = 629

      val scalaHex = toHex(simpleBytes)
      println("scalaHex: " + scalaHex)
      goSize shouldBe simpleBytes.length
      val goHex =
...
//    it("encode commit (java protobuf)") {
//      val simpleBytes = Amino.encodeJava(JavaAminoConverter.toAmino(block.last_commit))
//      val goSize = 629
//      goSize shouldBe simpleBytes.length
//
//      val scalaHex = toHex(simpleBytes)
...
//    }
    ...
    it("encode block") {
      val scalaHex = toHex(Amino.encodeLengthPrefixed(AminoConverter.toAmino(block)))
