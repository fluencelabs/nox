...
(xor
            ...
            (new $status
             (seq
              (seq
               ...
               (call n ("peer" "timeout") [5000 "timedout"] $status)
              )
              (xor
               (match $status.$.[0]! "done"
                (xor
                 (seq
                  (call -relay- ("op" "noop") [])
                  (xor
                   (call %init_peer_id% ("callbackSrv" "logStatus") ["success" n2 n])
                   (call %init_peer_id% ("errorHandlingSrv" "error") [%last_error% 2])
                  )
                 )
                 (call %init_peer_id% ("errorHandlingSrv" "error") [%last_error% 3])
               ...
               (xor
                (call %init_peer_id% ("callbackSrv" "logStatus") ["timed out" n2 n])
 ...
 (call %init_peer_id% ("errorHandlingSrv" "error") [%last_error% 7])
    ...
    let data = hashmap! {
        ...
        String::from("-relay-") => json!(swarms[1].peer_id.to_base58())
    };
    ...
    let closure = move |args: Args, _| {
        if args.service_id == "callbackSrv" && args.function_name == "logStatus" {
            ...
            // let from = args.function_args[2].as_str().unwrap().to_string();
    ...
    let result: eyre::Result<eyre::Result<_>> = block_on(timeout(Duration::from_secs(30), future));
    let nodes: HashMap<_, _> = result.unwrap().unwrap();
