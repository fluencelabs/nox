...
WAVM was a definite winner in every test we have performed, and "compilers" were up to 50x-200x times faster compared to "interpreters". Unfortunately, because we tried to run the same test on "compilers" and "interpreters", we couldn't make it run long enough for "compilers" without making "interpreters" to run forever.
