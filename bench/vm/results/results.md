...
We have used the following virtual machines versions:
...
All tests were performed on the AWS `m4.large` instance, and the script running benchmarks can be found [here](https://github.com/fluencelabs/fluence/tree/master/bench/vm/bencher).

It should be noted that a total execution time was measured, which skews results a little bit. For example, the compilation time by WAVM was counted, but ideally shouldn't because the compilation should happen only once. The time for JVM to load the class file produced by Asmble was counted as well. We attempted to counteract this by having tests to run long enough, but of course that's far from perfect.
...
It should be expected that compiler VMs will beat interpreters without JIT by large margin, and WAVM is a clear winner here. However, it's surprising to see that using `snappy` compression instead of `deflate` increases performance of compiler VMs almost twice, whereas it doesn't have too much effect on interpreter VMs. Also, `deflate` is one of two tests where Asmble loses to wasmer.

<p align="center">
  <img src="images/computations.png" alt="Computation tests" width="1001px"/>
</p>

Factorization is one of the tests where Asmble goes almost even with WAVM. We think that's because of the Java JIT optimizing things.
