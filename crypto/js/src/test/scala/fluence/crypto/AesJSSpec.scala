...
import fluence.crypto.algorithm.{ AesConfig, AesCryptJS, CryptoErr }
...
class AesSpec extends WordSpec with Matchers with slogging.LazyLogging {
  ...
  "aes crypto" should {
    "work with IV" in {
      ...
      val crypt = AesCryptJS.forString[Try](pass, withIV = true, config = conf)
      ...
      val aesWithoutIV = AesCryptJS.forString[Try](pass, withIV = false, config = conf)
    ...
    "work without IV" ignore {
      ...
      val fakeAes = AesCryptJS.forString[Try](ByteVector("wrong".getBytes()), withIV = false, config = conf)
      checkCryptoError(fakeAes.decrypt(crypted))

      //we cannot check if first bytes is iv or already data, but encryption goes wrong
      val aesWithIV = AesCryptJS.forString[Try](pass, withIV = true, config = conf)
