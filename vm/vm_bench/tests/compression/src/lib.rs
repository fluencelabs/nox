...
/// This test compresses sequence of SEQUENCE_SIZE bytes randomly generated by IsaacRng
/// in series. There are ITERATIONS_COUNT iterations of compressions. On each iteration a new
/// sequence is generated by seed that computed based on previously sequence. And as the first
/// seed SEED parameter is used. Also there are two supported compression algorithms (deflate
/// and snappy) that are chosen by "deflate_compression" feature.
///
/// Returns a final seed to prevent possible aggressive optimization.  
#[no_mangle]
