...
async fn add_script_from_vault_wrong_vault() {
    ...
    if let [JValue::String(error_msg)] = result.as_slice() {
        let expected_error_prefix = "Local service error, ret_code is 1, error message is '\"Error reading script file `/tmp/vault/another-particle-id/script`: Incorrect vault path";
        assert!(
