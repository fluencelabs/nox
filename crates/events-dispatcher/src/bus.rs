        ...
        for config in config.triggers {
            match config {
                TriggerConfig::Timer(config) => {
                    ...
                    self.scheduled.push(Scheduled::new(periodic, Instant::now()))
                },
...
fn test() {
    ...
    let (dispatcher, api, mut events) = SpellEventBus::new(cfg);
    dispatcher.start();

    let cfg = TriggerConfig {
        triggers: vec![
            TriggerConfig::PeerEvent(PeerEventConfig {
                events: vec![PeerEventType::Connect],
            }),
            TriggerConfig::Timer(TimerConfig {
