 ...
 */
class ConfigTemplate private (defaultTendermintDir: Path, tendermintConfig: TendermintConfig) extends LazyLogging {
  ...
  def writeConfigs[F[_]: LiftIO: Functor](app: App, appTendermintDir: Path, abciHost: String): F[Unit] =
    (
      for {
        defaultConfigs ← IO(defaultTendermintDir.resolve("config"))
        appConfigs ← IO(appTendermintDir.resolve("config"))

        defaultData ← IO(defaultTendermintDir.resolve("data"))
