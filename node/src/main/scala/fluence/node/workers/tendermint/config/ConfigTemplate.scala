 ...
 */
class ConfigTemplate(tendermintDir: Path, tendermintConfig: TendermintConfig) extends LazyLogging {
   ...
   */
  def writeConfigs[F[_]: LiftIO: Functor](app: App, appDataPath: Path, abciHost: String): F[Unit] =
    (
      for {
        defaultConfigs ← IO(tendermintDir.resolve("config"))
        appConfigs ← IO(appDataPath.resolve("config"))

        defaultData ← IO(tendermintDir.resolve("data"))
        appData ← IO(appDataPath.resolve("data"))
   ...
   */
  def copyMasterKeys(defaultConfigs: Path, appConfigs: Path): IO[Unit] = {
   ...
   */
  def updateConfigTOML(defaultConfigs: Path, appConfigs: Path, app: App, abciHost: String): IO[Unit] =
    for {
  ...
  /**
   * Copy `priv_validator_state.json` from `defaultData` to `appData`, without overwriting existing files.
   ...
   */
  def copyValidatorState(defaultData: Path, appData: Path): IO[Unit] = {
