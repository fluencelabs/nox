...
class ConfigTemplate(srcPath: Path) extends LazyLogging {
  ...
  /**
   * Takes `default_config.toml` from srcPath as a template, and writes it to destPath, making necessary substitutions
   *
   * @param destPath Target Tendermint's config dir
   * @param app Application to prepare
   * @param abciHost Usually Worker's name
   * @param abciPort ABCI port
   */
...
object ConfigTemplate {
   ...
   *        - config.toml, copied from `tpl.srcPath/default_config.toml` and updated
   */
