...
object Solver extends LazyLogging {
  ...
  * [[DockerIO.run]]'s command for launching a configured solver
    *
    * @param rpcPort the RPC port to bind to
    */
  private def command(rpcPort: Int): String = s"-p $rpcPort:80 nginx"
  ...
  * Runs a single solver
    ...
    */
  def run[F[_]: Concurrent: ContextShift: Timer](rpcPort: Int, healthcheck: HealthcheckConfig = HealthcheckConfig())(
        ...
        DockerIO
          ...
          .evalMap[F, (FiniteDuration, Either[String, Unit])] {
            case (d, true) ⇒
              ...
              logger.debug(s"Running HTTP healthcheck on port $rpcPort")
              sttp
                .get(uri"http://localhost:$rpcPort/${healthcheck.httpPath}")
                .send()
                .attempt
            ...
            case (d, false) ⇒
              logger.debug(s"HTTP healthcheck on port $rpcPort skipped, as container is not running")
          ...
          .compile
          .drain
          .map(_ ⇒ logger.debug(s"Finished with port $rpcPort"))
      )
    } yield Solver[F](ref, stop.complete(Right(())), fiber)
