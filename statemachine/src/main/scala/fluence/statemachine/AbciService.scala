...
import fluence.statemachine.data.{AbciServiceCodes, Tx, TxCode}
      ...
      case Some(head) ⇒
        ...
        state.get.map { st ⇒
          st.responses.find(_._1 == head) match {
            case Some((_, data)) ⇒
              QueryResponse(st.height, data, AbciServiceCodes.Ok, s"Responded for path $path")
