...
class TimeMeter(val startNanos: Long) extends AnyVal {
   ...
   */
  def millisElapsed: Long = NANOSECONDS.toMillis(System.nanoTime() - startNanos)
}
...
object TimeMeter {
  ...
  /**
   * Returns a number which is the amount of milliseconds from given value in past to now.
   *
   * @param millisInPast milliseconds timestamp measured at some instant in past
   */
  def millisFromPastToNow(millisInPast: Long): Long = System.currentTimeMillis() - millisInPast
}
