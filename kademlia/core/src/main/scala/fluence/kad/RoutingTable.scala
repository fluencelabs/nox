...
class RoutingTable[C](nodeId: Key)(implicit SR: Siblings.ReadOps[C], BR: Bucket.ReadOps[C])
  ...
  def updateList[F[_]: Monad: LiftIO, G[_]](
    ...
    pingExpiresIn: Duration,
    checkNode: Node[C] ⇒ IO[Boolean]
  )(implicit SW: Siblings.WriteOps[F, C], BW: Bucket.WriteOps[F, C], P: Parallel[F, G]): F[List[Node[C]]] = {
  ...
  def lookupIterative[F[_]: Monad: LiftIO, G[_]](
    ...
    pingExpiresIn: Duration,
    checkNode: Node[C] ⇒ IO[Boolean]
  )(implicit SW: Siblings.WriteOps[F, C], BW: Bucket.WriteOps[F, C], P: Parallel[F, G]): F[Seq[Node[C]]] = {
    // Import for Traverse
    import cats.instances.list._
    ...
    def advance(shortlist: SortedSet[Node[C]], probed: Set[Key]): F[AdvanceData] = {
      ...
      if (handle.isEmpty || shortlist.isEmpty) {
        ...
        remote0X
          .flatMap(updateList[F, G](_, rpc, pingExpiresIn, checkNode)) // Update routing table
          .map { remotes ⇒
  ...
  def callIterative[F[_]: Monad: LiftIO, G[_], E, A](
    ...
    pingExpiresIn: Duration,
    checkNode: Node[C] ⇒ IO[Boolean]
  )(implicit SW: Siblings.WriteOps[F, C], BW: Bucket.WriteOps[F, C], P: Parallel[F, G]): F[Vector[(Node[C], A)]] =
    lookupIterative[F, G](key, numToCollect max parallelism, parallelism, rpc, pingExpiresIn, checkNode).flatMap {
      prefetchedNodes ⇒
  ...
  def join[F[_]: Monad: LiftIO, G[_]](
    ...
    implicit SW: Siblings.WriteOps[F, C],
    BW: Bucket.WriteOps[F, C],
    P: Parallel[F, G]
  ): EitherT[F, RoutingTable.JoinError, Unit] =
    EitherT(
      Parallel
        .parTraverse(peers.toList) { peer: C ⇒
          ...
          rpc(peer).ping().attempt.to[F].flatMap[Option[(Node[C], List[Node[C]])]] {
            case Right(peerNode) if peerNode.key =!= nodeId ⇒ // Ping successful, lookup node's neighbors
              ...
              rpc(peer).lookup(nodeId, numberOfNodes).attempt.to[F].map {
        ...
        .flatMap { ns ⇒
          ...
          updateList[F, G](ns, rpc, pingExpiresIn, checkNode)
        }
        .map(_.nonEmpty)
        .flatMap[Either[RoutingTable.JoinError, Unit]] {
          case true ⇒ // At least joined to a single node
            logger.info("Joined! " + Console.GREEN + nodeId + Console.RESET)
            lookupIterative[F, G](nodeId, numberOfNodes, numberOfNodes, rpc, pingExpiresIn, checkNode)
              .map(_ ⇒ Right(()))
          case false ⇒ // Can't join to any node
            logger.warn("Can't join!")
            Monad[F].pure(Left[RoutingTable.JoinError, Unit](CantJoinAnyNode))
        }
    )
}
