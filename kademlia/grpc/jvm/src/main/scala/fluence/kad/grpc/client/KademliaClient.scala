...
class KademliaClient(stub: KademliaGrpc.Kademlia)(
  implicit
  codec: PureCodec[protocol.Node[Contact], kad.Node],
  ec: ExecutionContext
  ...
  private val streamCodec = PureCodec.codec[Stream[protocol.Node[Contact]], Stream[kad.Node]]
  ...
  override def ping(): IO[Node[Contact]] =
    for {
      n ← IO.fromFuture(IO(stub.ping(kad.PingRequest())))
  ...
  override def lookup(key: Key, numberOfNodes: Int): IO[Seq[Node[Contact]]] =
    for {
      k ← keyBS(key)
      res ← IO.fromFuture(IO(stub.lookup(kad.LookupRequest(k, numberOfNodes))))
  ...
  override def lookupAway(key: Key, moveAwayFrom: Key, numberOfNodes: Int): IO[Seq[Node[Contact]]] =
    for {
      ...
      res ← IO.fromFuture(
        IO(
          stub.lookupAway(kad.LookupAwayRequest(k, moveAwayK, numberOfNodes))
        )
      )
      resDec ← streamCodec.inverse.runF[IO](res.nodes.toStream)
    } yield resDec
