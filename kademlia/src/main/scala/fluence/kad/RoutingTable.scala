 ...
 */
class RoutingTable[F[_]: Monad, P[_], C](nodeId: Key, siblings: SiblingsState[F, C], buckets: BucketsState[F, C])(
  ...
  def lookupIterative(
    ...
    pingExpiresIn: Duration,
    checkNode: Node[C] ⇒ IO[Boolean]
  )(implicit clock: Clock[F], liftIO: LiftIO[F]): F[Seq[Node[C]]] = {
    // Import for Traverse
    import cats.instances.list._
  ...
  def callIterative[E, A](
    ...
    pingExpiresIn: Duration,
    checkNode: Node[C] ⇒ IO[Boolean]
  )(implicit clock: Clock[F], liftIO: LiftIO[F]): F[Vector[(Node[C], A)]] =
  ...
  def join(
    ...
    checkNode: Node[C] ⇒ IO[Boolean],
    parallelism: Int
  )(implicit clock: Clock[F], liftIO: LiftIO[F]): EitherT[F, RoutingTable.JoinError, Unit] =
    EitherT(
      Parallel
