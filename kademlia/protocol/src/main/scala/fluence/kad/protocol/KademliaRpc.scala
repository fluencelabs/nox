...
trait KademliaRpc[F[_], C] {
   ...
   */
  def ping(): EitherT[F, Error, Node[C]]
   ...
   */
  def lookup(key: Key, numberOfNodes: Int): EitherT[F, Error, Seq[Node[C]]]
   ...
   * @param key Key to lookup
   */
  def lookupAway(key: Key, moveAwayFrom: Key, numberOfNodes: Int): EitherT[F, Error, Seq[Node[C]]]
}

object KademliaRpc {
  type Aux[F[_], C, E] = KademliaRpc[F, C] { type Error = E }
}
