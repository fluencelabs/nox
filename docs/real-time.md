...
Every request made by the client is turned into a transaction which is then sent to one of Tendermint endpoints. For example, if the deployed WebAssembly code was produced from this Rust snippet: `fn sum(x: i32, y: i32) -> i32 { x + y }`, then the client might send a transaction looking like `{'fn': 'sum', 'x': 5, 'y': 3}` and await until `8` will be retrieved as a result.
